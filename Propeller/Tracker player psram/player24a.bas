...
'' -------------------- Enter and also file panel active - open and play the file
  ...
  if lcase$(right$(filename$,3))="wav" then  							' this is a wave file. Todo - read and use the header!
    ...
    lpoke base+16,$20000                                      					' loop end, we will use $50000 bytes as $50 4k buffers
    ...
    lpoke base+32+16,$20000                                       				' loop end
...
sub getwave
  ...
  '    psram.write(addr(wavebuf(0)), $100000+needbuf shl 14 ,$4000)
      needbuf=(needbuf+1) mod 8  								' we can have any count of 4k buffers, now 2 used
      wavepos+=$4000      									' file position
      endif
    if qqq<>$4000 then 										' end of file
      do: currentbuf=lpeek(base) shr 14 : loop until currentbuf=needbuf				' wait until all buffers played					
