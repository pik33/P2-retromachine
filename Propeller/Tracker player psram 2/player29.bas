...
let s1c=s1b>>1
if s1c<16 then lpoke v.palette_ptr+4*$F1,$00110000*((s1c+16)/2)             ' green
if s1c>=16 then lpoke v.palette_ptr+4*$F1,$00FF0000+(s1c-16)*$11000000	   ' green to yellow
if s1c>=32 then lpoke v.palette_ptr+4*$F1,$FFFF0000-(s1c-32)*$00220000	   ' yellow to red
if s1c>=48 then lpoke v.palette_ptr+4*$F1,$FF000000		           ' red

let s21c=s21b>>1
if s21c<16 then lpoke v.palette_ptr+4*$F2,$00110000*((s21c+16)/2)           ' the same for the rest of channels
if s21c>=16 then lpoke v.palette_ptr+4*$F2,$00FF0000+(s21c-16)*$11000000    ' using colors b,c,d,e from the palette
if s21c>=32 then lpoke v.palette_ptr+4*$F2,$FFFF0000-(s21c-32)*$00220000
if s21c>=48 then lpoke v.palette_ptr+4*$F2,$FF000000

let s31c=s31b>>1
if s31c<16 then lpoke v.palette_ptr+4*$F3,$00110000*((s31c+16)/2)
if s31c>=16 then lpoke v.palette_ptr+4*$F3,$00FF0000+(s31c-16)*$11000000
if s31c>=32 then lpoke v.palette_ptr+4*$F3,$FFFF0000-(s31c-32)*$00220000
if s31c>=48 then lpoke v.palette_ptr+4*$F3,$FF000000

let s41c=s41b>>1
if s41c<16 then lpoke v.palette_ptr+4*$F4,$00110000*((s41c+16)/2)
if s41c>=16 then lpoke v.palette_ptr+4*$F4,$00FF0000+(s41c-16)*$11000000
...
for ii=0 to s1b:  v.fastline(540+add,571+add,554-ii,$F1) : next ii: for ii=s1b+1 to 120 :v.fastline(540+add,571+add,554-ii,16): next ii 'cc=$bbbbbbbb : for jj=270 to 278 step 4: lpoke graphicbuf_ptr+448*(59-ii)+jj,cc : next jj: next ii ' now draw these bars
for ii=0 to s21b: v.fastline(586+add,617+add,554-ii,$F2) : next ii: for ii=s21b+1 to 120:v.fastline(586+add,617+add,554-ii,16): next ii 'cc=$bbbbbbbb : for jj=270 to 278 step 4: lpoke graphicbuf_ptr+448*(59-ii)+jj,cc : next jj: next ii ' now draw these bars
if (modplaying=1) or (dmpplaying=1) or (sidplaying=1) then for ii=0 to s31b: v.fastline(632+add,663+add,554-ii,$F3) : next ii: for ii=s31b+1 to 120:v.fastline(632+add,663+add,554-ii,16): next ii 'cc=$bbbbbbbb : for jj=270 to 278 step 4: lpoke graphicbuf_ptr+448*(59-ii)+jj,cc : next jj: next ii ' now draw these bars
if modplaying=1 then for ii=0 to s41b: v.fastline(678,709,554-ii,$F4) : next ii: for ii=s41b+1 to 120:v.fastline(678,709,554-ii,16): next ii 'cc=$bbbbbbbb : for jj=270 to 278 step 4: lpoke graphicbuf_ptr+448*(59-ii)+jj,cc : next jj: next ii ' now draw these bars
end sub
