...
'' --------------------------------  Playing the .wav file in the main loop as no other cogs can acces the file system
  ...
  if waveplaying=1 then
    qqq=$2000											' one wave chunk to load, 4kB=27 ms
    currentbuf=lpeek(base) shr 13								' get a current playing 4k buffer# from the driver
    if needbuf<>currentbuf then									' if there is a buffer to load
      get #8,wavepos,wavebuf(0),$2000,qqq 					' then load it
      psram.write(addr(wavebuf(0)), needbuf shl 12 ,$2000)
      needbuf=(needbuf+1) mod $80  								' we can have any count of 4k buffers, now 2 used
      wavepos+=$2000      									' file position
      endif
    if qqq<>$2000 then 										' end of file
...
'' -------------------- Enter and also file panel active - open and play the file
  ...
  if lcase$(right$(filename$,3))="wav" then  							' this is a wave file. Todo - read and use the header!
    ...
    close #8: open filename3$ for input as #8: get #8,1,wavebuf(0),$2000 
    psram.write(addr(wavebuf(0)),0,$2000)          						' open the file and preload the buffer
    needbuf=1: currentbuf=0 :wavepos=$2001 : waveplaying=1   					' init buffering variables
