...
'   position 3,17: v.write(v.inttostr2(pos-1,8)) 
  ...
  position 2,16:v.write(space$(38)): filename2$=right$(filename2$,38): position 2,15: v.write(filename2$)
'  position 2,17:v.write("Amiga module")
  ...
  if ansibuf(3)=asc("+") then mainvolume+=1: ansibuf(3)=0 : if mainvolume>128 then mainvolume=128 
...
'      filename$=left$(filename$,38)
      if i<11 then position 2,i : print filename$ 
      i+=1
    endif  
